...
tags = Gtag.where(event_id: 2).select { |tag| !tag.valid? }
tags.group_by(&:tag_uid).each do |_uid, tags|
...
credits = CustomerCredit.includes(:profile).where(profile: Event.find(2).profiles, transaction_origin: "refund", gtag_counter: nil)
credits.group_by(&:profile).each do |profile, online|
  last_onsite_index = (profile.customer_credits - online).sort_by { |c| c.gtag_counter.to_i }.last.gtag_counter
...
profiles = Profile.where(id: CreditTransaction.where(event_id: 2, transaction_type: %w(online_refund fee)).uniq.pluck(:profile_id)).includes(:customer_credits)
profiles.each do |p|
...
Event.find(2).profiles.select(:id).includes(:customer_credits, :credit_transactions, :access_transactions, :credential_transactions, :money_transactions, :order_transactions).find_in_batches.each do |profiles|
  profiles.each do |p|
    ...
    balance = 0
    credits.each_with_index do |credit, _index|
...
inconsistencies are ordered by inconsistency value
