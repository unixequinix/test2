class Refund < ApplicationRecord
  ...
  scope :xlsx_includes, -> { includes(:customer) }
  ...
  scope :online_refund_fee, lambda {
    select(:customer_id, "min(created_at) as date", "'fee' as action, 'refund_online' as description", dimensions_station, event_day_refund, date_time_refund, count_operations, "'c' as credit_name, NULL as device_name", "-1 * sum(credit_fee) as credit_amount")
