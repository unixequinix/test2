class Poke < ApplicationRecord
  ...
  scope :xlsx_includes, -> { includes(:device, :station, :customer, :customer_gtag, :operator, :operator_gtag, :ticket, :ticket_type, :product, :catalog_item) }
  ...
  scope :for_listings, -> { includes(:credit, :operation, :event, :station, :product).order(:gtag_counter, :line_counter, :date) }
  ...
  scope :with_customer, ->(customers) { customers.present? ? where(customer: customers) : all }
  ...
  scope :with_ticket_type, ->(ticket_types) { ticket_types.present? ? where(ticket_type: ticket_types) : all }
  scope :with_direction, ->(directions) { directions.present? ? where(access_direction: directions) : all }
  scope :with_priority, ->(priorities) { priorities.present? ? where(priority: priorities) : all }
  scope :with_staff, ->(staff) { staff.present? ? includes(:customer).where(customers: { operator: staff }) : all }

  # Money
  #
  scope :money_recon_simple, lambda {
    select("CASE WHEN action = 'topup' THEN 'income' ELSE action END as action, action as description", "min(date) as date", :payment_method, dimensions_customers, dimensions_operators_devices_simple, date_time_poke, dimensions_station, sum_money, count_operations)
      .joins(:station, :device, :operator, :operator_gtag)
      .has_money.is_ok.not_cash_recon
      .group(:action, :description, :payment_method, grouping_customers, grouping_operators_devices, grouping_station, "date_time")
      .having("sum(monetary_total_price) !=0")
  }

  scope :money_recon, lambda {
    select("CASE WHEN action = 'topup' THEN 'income' ELSE action END as action, action as description", "min(date) as date", :payment_method, date_time_poke, dimensions_customers, dimensions_operators_devices, dimensions_station, sum_money, count_operations)
  ...
  scope :top_products, lambda { |_credits = event.credits, limit = 10|
