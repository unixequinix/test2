class Event < ApplicationRecord
  ...
  has_many :bizzabo_ticketing_integrations, class_name: "TicketingIntegrationBizzabo", dependent: :destroy, inverse_of: :event
  ...
  after_update -> { sync_to_wiredlion(false) }
  ...
  validates :maximum_gtag_balance, :maximum_gtag_virtual_balance, :credit_step, numericality: { greater_than: 0, less_than_or_equal_to: 21_474_836 }
  ...
  delegate :currencies, to: :catalog_items
  ...
  def initial_setup!
    ...
    create_credit!(value: 1, name: "CRD", position: 1, spending_order: 1)
    create_virtual_credit!(value: 1, name: "Virtual", position: 2, spending_order: 2)
  ...
  def max_currencies_reached?
    (credits.count + tokens.count).eql?(MAX_CREDITS)
  end

  def sync_to_wiredlion(is_new = true)
    uri = URI("https://#{Rails.env.production? ? '' : 'demo.'}wiredlion.io/api/v1/events")
    http = Net::HTTP.new(uri.host, uri.port)
    http.use_ssl = true
    headers = { 'Authorization' => Rails.application.secrets.wiredlion_access_token, 'Content-Type' => 'application/json' }
    req = is_new ? Net::HTTP::Post.new(uri, headers) : Net::HTTP::Patch.new(uri, headers)
    req.body = Admins::Events::WiredlionEventSerializer.new(self).to_json
    res = http.request(req)
    response = res.code == '200' || res.code == '400' ? JSON.parse(res.body) : nil
    if response.present? && response['event'].present?
      self.wiredlion_event = response['event']['slug']
      save!
    end
    response
  end
  ...
  def wiredlion_url
    "https://#{Rails.env.production? ? '' : 'demo.'}wiredlion.io/dashboard/#{wiredlion_event}"
  end
