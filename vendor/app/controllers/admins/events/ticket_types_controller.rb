  ...
  module Events
    class TicketTypesController < Admins::Events::BaseController
      ...
      def index
        ...
        @ticket_counts_redeemed = @current_event.tickets.redeemed.group(:ticket_type_id).count
      ...
      def show
        ...
        @tickets = @q.result
        @tickets = @tickets.page(params[:page]) unless params[:format] == 'xls'
        respond_to do |format|
          format.html
          format.xls { handle_response(@current_event, @tickets, response, @ticket_type.name) }
        end
      end
