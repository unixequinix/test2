module Admins
  module Events
    class EventInvitationsController < Admins::Events::BaseController
      before_action :set_invitation, only: %i[update resend destroy accept]
      ...
      def create
        ...
        if !@current_event.users.map(&:email).include?(email)
          ...
          if @invitation.save
            ...
            redirect_to admins_event_event_registrations_path, alert: @invitation.valid? ? "Ups! Impossible to send the invitation" : "User already invited to the event"
          end
        else
          redirect_to admins_event_event_registrations_path, alert: "User is already registered and active"
        end
      end
