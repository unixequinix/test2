      ...
      class StationItemsController < Admins::Events::BaseController
        ...
        def create
          ...
          if @item.save
            ...
            flash.now[:alert] = @item.errors.full_messages.to_sentence || t("alerts.error")
            @transactions = []
            @devices = []
            @operators = 0
        ...
        def permitted_params
          params.require(params[:item_type]).permit(:direction, :access_id, :catalog_item_id, :name, :amount, :credit_id, :hidden, :position, :vat, :price, prices: {})
        end
      end
    end
  end
