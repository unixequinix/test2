    ...
    context "with authentication" do
      ...
      context "when the station is a point of sales" do
        before do
          ...
          @token = create(:token, event: event)
          create(:product, station: @station, prices: { event.credit.id.to_s => { price: rand(1.0...20.0).round(2) }, @token.id.to_s => { price: 9.1 } }, position: 9)
        end
        ...
        it "returns the catalog items for each pos" do
          ...
          s_db_items = @station.products.map { |p| { product_id: p.id, price: p.price, prices: p.prices.map { |k, v| { credit_id: k, spending_order: @station.event.currencies.find(k).spending_order }.merge(v) }, position: p.position, hidden: p.hidden }.as_json }
