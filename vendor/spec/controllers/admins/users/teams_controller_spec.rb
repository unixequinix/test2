...
RSpec.describe Admins::Users::TeamsController, type: :controller do
  ...
  describe "GET #move_devices" do
    context "user is not logged in" do
      it "redirects to sign_in path" do
        get :move_devices, params: { user_id: user.id }
        expect(response).to redirect_to(new_user_session_path)
      end
    end
    ...
    context "user is logged in" do
      ...
      context "user must be glowball" do
        it "redirect to admins user path on success" do
          user.update(team: create(:team, team_valid_attributes), role: 'promoter')
          get :move_devices, params: { user_id: user.id }
          expect(response).to redirect_to(admins_events_path)
        end
        it "get success for move devices path" do
          user.update(team: create(:team, team_valid_attributes), role: 'glowball')
          get :move_devices, params: { user_id: user.id }
  ...
  describe "PATCH #update_devices" do
    context "user is logged in and glowball" do
      before do
        @team1 = create(:team, team_valid_attributes)
        @team2 = create(:team, name: 'team2')
        @device = create(:device, device_valid_attributes.merge!(team: @team1))
        user.update(team: @team1, role: 'glowball')
        sign_in(user)
      end

      context "device can be moved" do
        it "device is associated with team1" do
          expect(@device.team).to eql(@team1)
        end

        context "as :js" do
          it "device is moved and associated to team2" do
            patch :update_devices, params: { user_id: user.id, device: { ids: [@device.id], destination_team_id: @team2.id, origin_team_id: @team1.id } }, format: :js
            expect(response).to be_ok
            @device.reload
            expect(@device.team).to eql(@team2)
          end
          it "device is not moved and associated to team2 if allowed is false" do
            create(:device_registration, event: create(:event), device: @device, allowed: false)
            patch :update_devices, params: { user_id: user.id, device: { ids: [@device.id], destination_team_id: @team2.id, origin_team_id: @team1.id } }, format: :js
            @device.reload
            expect(@device.team).to eql(@team1)
          end
