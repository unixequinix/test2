  ...
  describe "order_item dependent methods" do
    ...
    describe ".max_credit_reached" do
      it "adds an error if the credits exceeds the maximum allowed" do
        ...
        subject.save
        expect(subject.errors[:credits]).to include("have reached the maximum allowed balance")
        expect { subject }.not_to change(subject, :credits)
      end
      ...
      it "adds an error if the virtual credits exceeds the maximum allowed" do
        ...
        subject.save
        expect(subject.errors[:credits]).to include("have reached the maximum allowed balance")
        expect { subject }.not_to change(subject, :virtual_credits)
      end
    end
  end
end
