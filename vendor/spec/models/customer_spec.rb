...
RSpec.describe Customer, type: :model do
  ...
  describe "balance" do
    ...
    describe ".virtual_credits" do
      ...
      it "does not takes into account orders refunded" do
        create_list(:order, 2, customer: customer, status: "refunded", event: event)
        total = create_list(:order, 2, customer: customer, status: "completed", event: event).sum(&:virtual_credits)
        create(:order, customer: customer, status: "cancelled", event: event)
        expect(customer.virtual_credits).to eq(total)
      end
    ...
    describe ".tokens" do
      it "takes into account the gtag balance" do
        gtag.update!(final_tokens_balance: { token.id => 12 })
        expect(customer.tokens(token)).to eq(12)
        expect(customer.tokens).to eq(12)
      end

      it "takes into account orders completed" do
        total = create_list(:order, 3, customer: customer, status: "completed", event: event, tokens: 10).sum(&:tokens)
        create(:order, customer: customer, status: "cancelled", event: event)
        expect(customer.tokens).to eq(total)
      end
      ...
      it "does not take into account redeemed orders" do
        OrderItem.where(order: create_list(:order, 3, customer: customer, status: "completed", event: event, tokens: { token.id => 10 })).update_all(redeemed: true)
        expect(customer.tokens).to be_zero
      end
    end
  end
