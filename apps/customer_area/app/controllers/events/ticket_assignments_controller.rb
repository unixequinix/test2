class Events::TicketAssignmentsController < Events::BaseController
  ...
  def destroy
    ...
    @credit_log = CreditLog.create(customer_event_profile_id: current_customer_event_profile.id, transaction_type: CreditLog::TICKET_UNASSIGNMENT, amount: -@ticket_assignment.credentiable.company_ticket_type.preevent_product.preevent_product_items.joins(:preevent_item).where(preevent_items: {purchasable_type: "Credit"}).sum(:amount)) unless @ticket_assignment.credentiable.company_ticket_type.preevent_product.preevent_product_items.joins(:preevent_item).where(preevent_items: {purchasable_type: "Credit"}).blank?
