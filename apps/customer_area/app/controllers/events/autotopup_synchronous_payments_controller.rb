class Events::AutotopupSynchronousPaymentsController < Events::PaymentsBaseController
  def create
    ...
    payer = "Payments::Autotopup#{payment_service.camelize}Payer".constantize.new
    if payer.start(params, CustomerOrderCreator.new, CustomerCreditOrderCreator.new)
      redirect_to success_event_order_payment_service_synchronous_payments_path(current_event,
                                                                                params[:order_id],
                                                                                payment_service)
    else
