...
def create
  ...
  atts = new_params.merge(birthdate: date, recaptcha: verify_recaptcha(model: @customer_form))
...
def update
  ...
  @edit_profile_form = EditProfileForm.new(current_customer)
  if @edit_profile_form.validate(edit_params.merge!(birthdate: date)) && @edit_profile_form.save
