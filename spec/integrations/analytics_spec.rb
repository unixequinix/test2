...
RSpec.shared_examples "kpis analytics" do
  ...
  it "shows the outstanding credits" do
    ourstanding = @topups.sum(&:credit_amount) + @sales.sum(&:credit_amount)
    expect(find("#outstanding_credits_kpi").text).to include(ourstanding.to_f.to_s)
  end
...
RSpec.describe "Analytics in the admin panel", type: :feature do
  ...
  context "visit pages on analytics dashboard" do
    before(:each) do
      visit admins_event_analytics_path(event)
    end
