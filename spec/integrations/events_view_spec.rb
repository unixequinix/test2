...
RSpec.describe "Test on Events view", js:true, type: :feature do
  ...
  describe "filter events by state" do
    ...
    it "click on 'created' label" do
      ...
      within 'table' do
        expect(page).not_to have_text "#{@event_launched.name}"
        expect(page).to have_text "#{@event_created.name}"
        expect(page).not_to have_text "#{@event_closed.name}"
      end
    end
    ...
    it "click on 'closed' label" do
      ...
      within 'table' do
        expect(page).not_to have_text "#{@event_launched.name}"
        expect(page).not_to have_text "#{@event_created.name}"
        expect(page).to have_text "#{@event_closed.name}"
      end
    end
  ...
  describe "search event" do
    ...
    it "type event name" do
      ...
      within("#event_search") {fill_in 'fixed-header-drawer-exp', with: "#{@event_created.name}"}.native.send_keys(:return)
      within 'table' do
        expect(page).not_to have_text "#{@event_launched.name}"
        expect(page).to have_text "#{@event_created.name}"
        expect(page).not_to have_text "#{@event_closed.name}"
  ...
  describe "actions in events view" do
    ...
    it "select an event" do
      find_by_id("#{@event_launched.id}_link").click
