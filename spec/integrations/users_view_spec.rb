...
RSpec.describe "Users view", js: true, type: :feature do
  ...
  describe "users view link" do
    ...
    it "all users are displayed" do
      ...
      within 'table' do
        expect(page).to have_text @user_glowball.username.to_s
        expect(page).to have_text @user_admin.username.to_s
      end
    end
  end
  ...
  describe "actions in user view" do
    ...
    it "select an user" do
      find("##{@user_admin.id}_email").click
    ...
    it "delete an user" do
      expect do
        find("##{@user_admin.id}_delete").click
        page.accept_alert
        sleep(1)
  ...
  describe "search user" do
    ...
    it "type username" do
      ...
      within("#user_search") { fill_in 'fixed-header-drawer-exp', with: @user_admin.username.to_s }.native.send_keys(:return)
      within 'table' do
        expect(page).not_to have_text @user_glowball.username.to_s
        expect(page).to have_text @user_admin.username.to_s
      end
    end
    ...
    it "type email" do
      find("#search_icon").click
      within("#user_search") { fill_in 'fixed-header-drawer-exp', with: @user_admin.email.to_s }.native.send_keys(:return)
      within 'table' do
        expect(page).not_to have_text @user_glowball.email.to_s
        expect(page).to have_text @user_admin.email.to_s
      end
    end
    ...
    it "type non-existent user name" do
      ...
      within 'table' do
        expect(page).not_to have_text @user_glowball.username.to_s
        expect(page).not_to have_text @user_admin.username.to_s
      end
    end
