...
RSpec.describe Creators::CustomerJob, type: :job do
  ...
  context "copying customer" do
    it "can copy customer" do
      expect { worker.perform_now(old_customer, event, "0") }.to change { event.customers.count }.by(1)
    end
    it "can add balances to customer" do
      gtag.credits = 5.to_f
      gtag.save!
      expect { worker.perform_now(old_customer, event, "0") }.to change { old_customer.credits }.by(5)
    end
  end
  context "creating orders" do
    it "can create order with credits and virtual" do
      ...
      gtag.save!
      expect { worker.perform_now(old_customer, event, "1") }.to change { event.orders.count }.by(1)
    ...
    it "can create order with credits and virtual but not virtual" do
      ...
      gtag.save!
      expect { worker.perform_now(old_customer, event, "0") }.to change { event.orders.count }.by(1)
    ...
    it "can't create order with virtual" do
      ...
      gtag.save!
      expect { worker.perform_now(old_customer, event, "0") }.to change { event.orders.count }.by(0)
    end
    it "can't create order without credits" do
      gtag.credits = 0.to_f
      gtag.virtual_credits = 0.to_f
      gtag.save!
      expect { worker.perform_now(old_customer, event, "0") }.not_to change(Order, :count)
      expect { worker.perform_now(old_customer, event, "1") }.not_to change(Order, :count)
    end
  end
end
