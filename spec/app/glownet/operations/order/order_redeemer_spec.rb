...
RSpec.describe Operations::Order::OrderRedeemer, type: :job do
  let(:event) { create(:event) }
  let(:worker) { Operations::Order::OrderRedeemer }
  let(:gtag) { create(:gtag, tag_uid: "FOOBARBAZ", event: event) }
  let(:profile) { create(:profile) }
  let(:catalog_item) { create(:standard_credit_catalog_item, event: event) }
