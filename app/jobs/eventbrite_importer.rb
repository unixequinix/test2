  ...
  def perform(order, event_id)
    ...
    order[:attendees].each do |guest|
      guest["barcodes"].each do |barcode|
        ...
        begin
          ctt = TicketType.find_or_initialize_by(company_code: guest["ticket_class_id"], event_id: event_id)
          ctt.update(name: guest["ticket_class_name"])
        rescue ActiveRecord::RecordNotUnique
          retry
        end
        begin
          ticket = Ticket.find_or_initialize_by(company: "Eventbrite - #{order[:event_id]}", code: barcode["barcode"], ticket_type: ctt, event_id: event_id)
