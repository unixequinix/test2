  ...
  class TicketMasterImporter < ApplicationJob
    ...
    def perform(event_code, event_data, integration) # rubocop:disable Metrics/AbcSize
      ...
      if response['command1']['events'].present?
        ...
        ticket_types_data.values.map { |tt| integration.ticket_types.find_or_create_by!(company: company, company_code: tt[:code], ticketing_integration_id: integration.id, event_id: integration.event_id, name: name) }
