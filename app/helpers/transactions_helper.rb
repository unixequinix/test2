module TransactionsHelper
  ...
  def apply_customers(event_id, customer_id, credential)
    claimed = Customer.claim(event_id, customer_id, credential.customer_id)
    return customer_id if claimed.present?

    credential.update!(customer_id: customer_id) if customer_id.present?
    credential.update!(customer_id: Customer.create!(event_id: event_id, anonymous: true).id) if credential.customer_id.blank?
