class TicketingIntegrationQwantiq < TicketingIntegration
  belongs_to :event, inverse_of: :qwantiq_ticketing_integrations
  ...
  def remote_events
    api_response(URI("#{GlownetWeb.config.qwantiq_venues_url}?sVenues=#{venue}"))
  end
  ...
  def import
    ...
    url = URI("#{GlownetWeb.config.qwantiq_barcodes_url}?#{params.to_param}")

    update!(last_import_date: Time.zone.now - 1.minute)

    response = api_response(url)
    response&.each { |ticket| Ticketing::QwantiqImporter.perform_later(ticket, self) }
  end
end
