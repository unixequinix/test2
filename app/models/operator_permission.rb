class OperatorPermission < CatalogItem
  has_many :operator_transactions, dependent: :destroy, foreign_key: :catalog_item_id, inverse_of: :catalog_item
  ...
  validates :role, uniqueness: { scope: %i[event_id group station_id], message: (->(object, _data) { "This role has already been used for #{'group ' + object.group if object.group}#{'station ' + object.station.name if object.station}" }) } # rubocop:disable Metrics/LineLength
